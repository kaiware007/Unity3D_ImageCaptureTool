...
public class ImageCaptureTool : EditorWindow
{
    ...
    private void CaptureImage()
    {
        ...
        // スクリーン上のレンダリングを終えるのを待機します。
        // http://docs.unity3d.com/jp/current/ScriptReference/WaitForEndOfFrame.html
    ...
    private Texture2D GenerateCaptureImage()
    {
        ...
        Texture2D captureImage
            = new Texture2D(fixedWidth, fixedHiehgt, TextureFormat.ARGB32, false);

        fixedCamera.Render();

       // アクティブなテクスチャを出力用にレンダリングしたテクスチャに切り替え、
       // Texture2D に読み込んで適用します。このとき MipMap を再計算しません。
